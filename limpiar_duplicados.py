...
import sys
sys.path.insert(0, '.')
from robot_buscar_temas import similitud_titulares, actorMencionadoEn, cargar_actores_alta_influencia, RUTA_EVENTOS, RUTA_TEMAS
...
def limpiar():
    ...
    for i, ev in enumerate(eventos):
        ...
        for j in range(i+1, len(eventos)):
            ...
            if similitud_titulares(ev['descripcion'], otro['descripcion']) >= UMBRAL:
                ...
                usados.add(j)
                continue
            en_ev = {a['nombre'] for a in actores_altos if actorMencionadoEn(a['nombre'], ev['descripcion'].lower())}
            en_otro = {a['nombre'] for a in actores_altos if actorMencionadoEn(a['nombre'], otro['descripcion'].lower())}
            if len(en_ev & en_otro) >= 2:
                grupo.append(otro)
                usados.add(j)
    ...
    with open(RUTA_EVENTOS, 'w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=campos_ev, quoting=csv.QUOTE_MINIMAL, extrasaction='ignore')
        w.writeheader()
    ...
    with open(RUTA_TEMAS, 'w', encoding='utf-8', newline='') as f:
        w = csv.DictWriter(f, fieldnames=campos_tm, quoting=csv.QUOTE_MINIMAL, extrasaction='ignore')
        w.writeheader()
